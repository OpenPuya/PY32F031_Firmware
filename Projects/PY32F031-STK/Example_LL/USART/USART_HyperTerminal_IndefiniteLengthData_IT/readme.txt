...
（不超过200byte），例如0x1~0xC,则MCU会把接收到的数据再次发送到上位机。
...
send any length of data (not exceeding 200bytes) through the upper computer,
...
4. 上位机发送任意长度个数据（不超过200byte），MCU会将接收到的数据回发给上位机
...
4. The upper computer sends any length of data (not exceeding 200 bytes), and
the MCU will send the received data back to the upper computer
================================================================================
注意事项：
用户可根据需要修改main.h中宏RX_MAX_LEN、RX_TIMEOUT的值，RX_MAX_LEN定义了MCU单次
接收数据长度（当前样例为200byte），RX_TIMEOUT定义了MCU单次接收数据的超时时间（当
前样例为2ms）。

Notes:
Users can modify the values of macros RX_MAX_LEN and RX_TIMEOUT in main.h as
needed. RX_MAX_LEN defines the length of data received by MCU for a single time
(currently 200bytes), and RX_TIMEOUT defines the timeout time for MCU to receive
data for a single time (currently 2ms).
================================================================================